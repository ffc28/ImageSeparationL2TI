...
V_recto = dico_recto.fit(patches).components_
dt = time() - t0
...
V_verso = dico_verso.fit(patches).components_
dt = time() - t0
print('done in %.2fs.' % dt)
...
code_verso = dico_verso.transform(data)

patch = np.dot(code_verso, V_verso)
patch += intercept
patch = np.reshape(patch, initial_patch_size)

im_re_verso = unpatchify(np.asarray(patch), image_size)
print('done in %.2fs.' % (time() - t0))

difference_verso = img1_gray - im_re_verso
print('Difference is ', np.sqrt(np.sum(difference_verso ** 2)))


#######################################
# Then I flip the image
img1_gray = np.fliplr(img1_gray)
print('Extracting noisy patches... ')
t0 = time()
data = patchify(img1_gray, patch_size, step)
data = data.reshape(-1, patch_size[0] * patch_size[1])
